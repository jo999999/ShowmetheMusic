...
# 누를 때마다 재추천한 값을 리턴 하는 함수. (출력값은 저장)
...
# 통합
def all_rs(emotion_list, text, weights):
                 # 함수: recsys_step3까지
                 # (입력:정/양방향 감정에 따른 노래 선택 입력 받기)
    df =         # 함수: 정/양방향 감정에 따른 노래 선택 반영한 1차 추천
    show(df) # 1차 추천 결과인 Top5노래 보여주기 (함수인데 만들었음)
            # (입력: 5점 평점 받기)
    a = int(input('추천 받기 = 1, 추천 멈추기 = 0', ))  # 0입력시 추천 종료
    while a:
        df = reco2(df) # 함수: 5점 평점 반영한 재추천
